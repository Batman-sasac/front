...
const PREVIEW_HEIGHT = Math.min(scale(380), WINDOW_HEIGHT * 0.45);
...
export default function SelectPicture({
  ...
  const [cropByIndex, setCropByIndex] = useState<
    Record<number, { px: number; py: number; pw: number; ph: number }>
  >({});
  ...
  useEffect(() => {
    ...
    containerRef.current = { w: containerW, h: containerH };
    console.log("🖼️ Crop 초기화:", { x, y, w, h }, "Container:", {
      w: containerW,
      h: containerH,
    });
  }, [
  ...
  const handleStart = () => {
    ...
    const limitReached =
      !isUnlimitedUser &&
      (ocrUsage?.status === "limit_reached" || remaining <= 0);
  ...
  const handleAddCategory = async () => {
    ...
    const nextCategories = [...customCategories, nextName];
  ...
  const handleDeleteCategory = async () => {
    ...
    const nextCategories = customCategories.filter(
      (category) => category !== editingCategoryName,
    );
    await saveCustomCategoryList(nextCategories);
    if (subjectName === editingCategoryName) {
      setSubjectName("");
  ...
  const tlResponder = useRef<PanResponderInstance>(
    createResizeResponder("tl"),
  ).current;
  const trResponder = useRef<PanResponderInstance>(
    createResizeResponder("tr"),
  ).current;
  const blResponder = useRef<PanResponderInstance>(
    createResizeResponder("bl"),
  ).current;
  const brResponder = useRef<PanResponderInstance>(
    createResizeResponder("br"),
  ).current;
  ...
  const exceedsRemainingOcr =
    !isUnlimitedUser && !!ocrUsage && sources.length > remainingOcr;
  ...
  const isReadyToStart =
    !!selectedSource && isCategoryReady && (!isSelectedImage || isCropUiReady);
  ...
  const renderSourcePreview = (source: StudySource, isThumb = false) => {
    ...
    return (
      <View style={isThumb ? styles.fileThumb : styles.fileCard}>
        ...
        <Text
          style={isThumb ? styles.fileThumbName : styles.fileName}
          numberOfLines={isThumb ? 2 : 3}
        >
              ...
              {allCategories.map((category) => {
                ...
                return (
                  <Pressable
                    ...
                    onPress={() => setSubjectName(category)}
                    onLongPress={
                      isCustomCategory
                        ? () => {
              ...
              >
                ...
                {isSelectedImage && isCropUiReady && (
                  <View style={styles.cropArea}>
                    ...
                    >
                      <View
                        style={[styles.cropCornerTL, { pointerEvents: "none" }]}
                      />
                      <View
                        style={[styles.cropCornerTR, { pointerEvents: "none" }]}
                      />
                      <View
                        style={[styles.cropCornerBL, { pointerEvents: "none" }]}
                      />
                      <View
                        style={[styles.cropCornerBR, { pointerEvents: "none" }]}
                      />
                    </View>

                    {/* 크롭 프레임 이동 영역 (투명한 내부 영역) */}
                    <View
                      style={[styles.cropMoveArea, overlayStyles.frame]}
                    ...
                    <View
                      style={[
                        styles.handle,
                        {
                          left: overlayStyles.frame.left - 12,
                          top: overlayStyles.frame.top - 12,
                        },
                      ]}
                      {...tlResponder.panHandlers}
                    >
                      <View
                        style={[styles.handleDot, { pointerEvents: "none" }]}
                      />
                    </View>
                    <View
                      style={[
                        ...
                        {
                          left:
                            overlayStyles.frame.left +
                            overlayStyles.frame.width -
                            12,
                          top: overlayStyles.frame.top - 12,
                        },
                      ]}
                    ...
                    <View
                      style={[
                        styles.handle,
                        {
                          left: overlayStyles.frame.left - 12,
                          top:
                            overlayStyles.frame.top +
                            overlayStyles.frame.height -
                            12,
                        },
                      ]}
