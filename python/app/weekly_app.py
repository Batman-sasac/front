...
async def get_weekly_growth(user_email: str = Cookie(None)):
    ...
    try:
        ...
        # SQLite에서는 DATE_TRUNC 대신 strftime 사용
        cur.execute("""
            WITH weekly_stats AS (
                SELECT 
                    strftime('%Y-%W', created_at) as week_start,
            ...
            SELECT 
                week_start,
                MIN((activity_score * (attend_days / 7.0)), 100) as growth_score
            FROM weekly_stats
...
async def get_learning_stats(user_email: str = Cookie(None)):
    ...
    try:
        ...
        cur.execute("""
            ...
            WHERE user_email = ? AND completed_at >= ?
        ...
        cur.execute("""
            SELECT COUNT(*) FROM study_logs 
            WHERE user_email = ? AND completed_at >= ? AND completed_at < ?
        """, (user_email, last_month_start, this_month_start))
        last_month_count = cur.fetchone()[0]

        # 4. 목표 횟수 가져오기 (users 테이블)
        cur.execute("SELECT target_count FROM users WHERE email = ?", (user_email, ))
        row = cur.fetchone()
