...
async def get_user_info(token: str):
    ...
    try:
        # JWT 토큰 검증
        from app.security_app import verify_jwt_token
        payload = verify_jwt_token(token)
        email = payload.get("email")
        
        if not email:
            return JSONResponse(status_code=401, content={"error": "유효하지 않은 토큰"})
        ...
        # 현재는 단일 소셜 로그인만 지원하므로, 로그인한 계정의 이메일을 반환
        # social_id가 있으면 카카오 로그인으로 가정 (추후 provider 필드 추가 가능)
        return {
            "status": "success",
            "email": user.get("email", ""),
            "nickname": user.get("nickname", ""),
            "kakao_connected": bool(user.get("social_id")),  # social_id가 있으면 카카오 연결된 것
            "naver_connected": False,  # 현재 네이버는 미구현
            "kakao_email": user.get("email", "") if user.get("social_id") else None,
            "naver_email": None,
        }
...
async def set_nickname_mobile(email: str = Form(...), nickname: str = Form(...)):
    """신규 유저의 닉네임 설정 (토큰 없이 email로 설정)"""
    try:
        # 1. 닉네임 중복 체크
        check_res = supabase.table("users").select("email").eq("nickname", nickname).execute()
        
        if check_res.data:
            return JSONResponse(
                status_code=400,
                content={"error": "이미 사용 중인 닉네임입니다."}
            )
        
        # 2. 닉네임 업데이트
        update_res = supabase.table("users").update({"nickname": nickname}).eq("email", email).execute()
        
        if not update_res.data:
            return JSONResponse(status_code=404, content={"error": "사용자를 찾을 수 없습니다"})
        
        # 3. social_id 조회해서 토큰 발급
        user = update_res.data[0]
        social_id = user.get("social_id")
        
        token = create_jwt_token(email, social_id)
        
        print(f"[닉네임 설정] {email} -> {nickname}")
