...
class CLOVAOCRService:
    ...
    def extract_text_with_clova(self, file_bytes, filename):
        ...
        try:
            ...
            # Clova OCR이 지원하는 포맷으로 변환
            # 허용 포맷: jpg, jpeg, png, tif, tiff, pdf
            format_mapping = {
                'jpg': 'jpg',
                'jpeg': 'jpeg',
                'png': 'png',
                ...
                'tiff': 'tiff',
                'pdf': 'pdf',
                'webp': 'png',  # webp는 png로 변환 필요
                'heic': 'jpg',  # heic는 jpg로 변환 필요
            }
            
            clova_format = format_mapping.get(file_ext, 'jpg')  # 기본값 jpg
            print(f"[Clova OCR] 원본 포맷: {file_ext}, Clova 포맷: {clova_format}")
            
            # 클로바 OCR 요청 데이터 구성
            request_json = {
                'images': [{'format': clova_format, 'name': 'ocr_request'}],
